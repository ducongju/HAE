...
def _PAD(inp, params):
    ...
    group_list = list(range(params.num_joints))
    ...
    for i in range(params.num_joints):
        ...
        # 当前关键点的信息
        tags = tag_k[idx_temp]
        joints = np.concatenate((loc_k[idx_temp], val_k[idx_temp, :, None], tags), 1)
        mask = joints[:, 2] > params.detection_threshold
        tags = tags[mask]
        joints = joints[mask]

        if joints.shape[0] == 0:
            idx_ = group_list.index(idx_temp)
            del group_list[idx_]
            continue
        ...
        if i == 0 or len(joint_dict) == 0:
            for tag, joint in zip(tags, joints):
                key = tag[0]
                joint_dict.setdefault(key, np.copy(default_))[i] = joint
                tag_dict[key] = [tag]
            ...
            for idx in group_list:
                # 每个关键点的信息
                tags_temp = tag_k[idx]
                joints_temp = np.concatenate((loc_k[idx], val_k[idx, :, None], tags_temp), 1)
                mask_temp = joints_temp[:, 2] > params.detection_threshold
                tags_temp = tags_temp[mask_temp]
                joints_temp = joints_temp[mask_temp]

                # 如果没有该关键点，则设置一个很大的匹配损失
                if joints_temp.shape[0] == 0:
                    diff_min.append(10)
                    continue

                grouped_keys_temp = list(joint_dict.keys())[:params.max_num_people]
                grouped_tags_temp = [np.mean(tag_dict[i], axis=0) for i in grouped_keys_temp]

                if (params.ignore_too_much
                        and len(grouped_keys_temp) == params.max_num_people):
                    continue

                diff_temp = joints_temp[:, None, 3:] - np.array(grouped_tags_temp)[None, :, :]
                diff_normed_temp = np.linalg.norm(diff_temp, ord=2, axis=2)
                # 以最小的tag距离作为标准，存入每个关键点的最小diff值
                diff_min.append(np.min(diff_normed_temp))
            
            # 获取group_list中最小tag距离对应的关键点，以该关键点作为这次循环的关键点
            idx_min = diff_min.index(min(diff_min))

            # 将group_list的位置转化为tag_k里的位置
            idx_tag = group_list[idx_min]

            # 选择该关键点进行字典更新
            tags = tag_k[idx_tag]
            joints = np.concatenate((loc_k[idx_tag], val_k[idx_tag, :, None], tags), 1)
            mask = joints[:, 2] > params.detection_threshold
            tags = tags[mask]
            joints = joints[mask]
            ...
            for row, col in pairs:
                if (row < num_added and col < num_grouped
                    ...
                    joint_dict[key][idx_tag] = joints[row]
                    tag_dict[key].append(tags[row])
                else:
                    key = tags[row][0]
                    joint_dict.setdefault(key, np.copy(default_))[idx_tag] = \
                        joints[row]
                    tag_dict[key] = [tags[row]]

            # 将该关键点从列表中移除，那么下次循环时就不会有这个关键点了
            del group_list[idx_min]
